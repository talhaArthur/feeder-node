...
test('bufferThreshold getter returns backend bufferThreshold', () => {
	let ctx = new AudioContext();
	let nChannels = 2;
	let batchSize = 512;
	let bufferLength = 192000;
	...
	expect(feederNode.bufferThreshold).toBe(16384);
});

test('setBufferThreshold changes the buffer threshold', () => {
	let ctx = new AudioContext();
	let nChannels = 2;
	let batchSize = 512;
	let bufferLength = 192000;
	...
	feederNode.setBufferThreshold(32768);
	
	// Check the new value
	expect(feederNode.bufferThreshold).toBe(32768);
});

test('setBufferThreshold throws error if threshold < 0', () => {
	let ctx = new AudioContext();
	let nChannels = 2;
	let batchSize = 512;
	let bufferLength = 192000;
	...
	// Expect an error when setting negative threshold
	expect(() => {
		feederNode.setBufferThreshold(-1);
	}).toThrow("bufferThreshold cannot be less than 0");
});

test('setBufferThreshold throws error if threshold > bufferLength', () => {
	let ctx = new AudioContext();
	let nChannels = 2;
	let batchSize = 512;
	let bufferLength = 192000;
	...
	// Expect an error when setting threshold greater than buffer length
	expect(() => {
		feederNode.setBufferThreshold(192001);
	}).toThrow("bufferThreshold cannot be greater than bufferLength");
});
